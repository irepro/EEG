...
class Resnet50Encoder(nn.Module):
    def __init__(self, inchannels = 12, n_classes=22):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv1d(inchannels, 128, kernel_size = 5, stride = 2, padding = 2 ), # Code overlaps with previous assignments
            nn.BatchNorm1d(128),
            nn.ReLU(inplace=True)
        )
        ...
        self.layer3 = nn.Sequential(
            ResidualBlock(156, 156, 156),
            ResidualBlock(156, 172, 172, downsample=True),
            ResidualBlock(172, 172, 172),
            ResidualBlock(172, 196, 196,downsample=True) # Code overlaps with previous assignments
        )
        self.layer4 = nn.Sequential(
            ResidualBlock(196, 196, 196),
            ResidualBlock(196, 224, 224, downsample=True),
            ResidualBlock(224, 224, 224),
            ResidualBlock(224, 256, 256, downsample=True) # Code overlaps with previous assignments
        )
        ...
        self.bridge = conv(256, 256)
        self.fullconnect = nn.Linear(256,2)
        # Dropout
